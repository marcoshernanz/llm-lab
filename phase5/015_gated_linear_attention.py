"""Phase 5 experiment 015: the decoder with gated linear attention on the local layers."""
...
class KimiDeltaAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:  # [B, T, D]
        ...
        k = self.k_norm(split_heads(self.k_proj(x), NUM_HEADS, D_HEAD))  # [B, Hkv, T, Dh]
        v = split_heads(self.v_proj(x), NUM_HEADS, D_HEAD)  # [B, Hq, T, Dh]
