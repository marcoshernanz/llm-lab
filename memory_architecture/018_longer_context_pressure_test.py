"""Memory architecture experiment 018: longer-context pressure test."""

from __future__ import annotations

import math
import os
from pathlib import Path
import random

import torch
import torch.nn.functional as F
from torch import nn

from lib.memory_plotting import MemoryMetricTracker

EXPERIMENT_NAME = os.environ.get("LLM_LAB_EXPERIMENT_NAME", Path(__file__).stem)
SEED = int(os.environ.get("LLM_LAB_SEED", "1337"))

MODEL_KIND = os.environ.get("LLM_LAB_MODEL_KIND", "memory")
CHUNK_SIZE = int(os.environ.get("LLM_LAB_CHUNK_SIZE", "16"))
SEQUENCE_LEN = int(os.environ.get("LLM_LAB_SEQUENCE_LEN", "256"))
assert SEQUENCE_LEN % CHUNK_SIZE == 0
EMBEDDING_DIM = 64
NUM_HEADS = 4
assert EMBEDDING_DIM % NUM_HEADS == 0
ADDRESS_DIM = 32
NUM_MEMORY_SLOTS = int(os.environ.get("LLM_LAB_NUM_MEMORY_SLOTS", "64"))
TOP_K_MEMORY_READS = int(os.environ.get("LLM_LAB_TOP_K_MEMORY_READS", "8"))
TOP_K_MEMORY_WRITES = int(os.environ.get("LLM_LAB_TOP_K_MEMORY_WRITES", "8"))
READ_TEMPERATURE = 0.25
WRITE_TEMPERATURE = 0.25
ADDRESS_UPDATE_SCALE = float(os.environ.get("LLM_LAB_ADDRESS_UPDATE_SCALE", "0.20"))
ADDRESS_INIT_MODE = os.environ.get("LLM_LAB_ADDRESS_INIT_MODE", "learned_random")
ALLOCATION_MODE = os.environ.get("LLM_LAB_ALLOCATION_MODE", "off")
ALLOCATION_STRENGTH = float(os.environ.get("LLM_LAB_ALLOCATION_STRENGTH", "1.0"))
ALLOCATION_TEMPERATURE = float(os.environ.get("LLM_LAB_ALLOCATION_TEMPERATURE", "0.25"))
USAGE_DECAY = float(os.environ.get("LLM_LAB_USAGE_DECAY", "1.0"))
WRITE_GRANULARITY = os.environ.get("LLM_LAB_WRITE_GRANULARITY", "chunk")
HIDDEN_DIM = 256
NUM_BLOCKS = 4

BATCH_SIZE = 64
LEARNING_RATE = 3e-3
TRAIN_STEPS = int(os.environ.get("LLM_LAB_TRAIN_STEPS", "6000"))
EVAL_INTERVAL = int(os.environ.get("LLM_LAB_EVAL_INTERVAL", "300"))
EVAL_BATCHES = int(os.environ.get("LLM_LAB_EVAL_BATCHES", "32"))

NUM_FACTS = int(os.environ.get("LLM_LAB_NUM_FACTS", "12"))
NUM_KEYS = int(os.environ.get("LLM_LAB_NUM_KEYS", "24"))
NUM_VALUES = int(os.environ.get("LLM_LAB_NUM_VALUES", "24"))
NUM_NOISE_TOKENS = int(os.environ.get("LLM_LAB_NUM_NOISE_TOKENS", "48"))
CANDIDATE_GUESS_BASELINE = 1.0 / NUM_FACTS
RANDOM_VALUE_EXACT_BASELINE = 1.0 / NUM_VALUES
RANDOM_VALUE_CANDIDATE_BASELINE = NUM_FACTS / NUM_VALUES

PAD_TOKEN = "[PAD]"
BOS_TOKEN = "[BOS]"
EOS_TOKEN = "[EOS]"
STORE_TOKEN = "[STORE]"
QUERY_TOKEN = "[QUERY]"
ANSWER_TOKEN = "[ANSWER]"


def choose_device() -> str:
    """Return the requested accelerator, then CUDA, then MPS, then CPU."""
    requested_device = os.environ.get("LLM_LAB_DEVICE")
    if requested_device:
        return requested_device
    if torch.cuda.is_available():
        return "cuda"
    if torch.backends.mps.is_available():
        return "mps"
    return "cpu"


DEVICE = choose_device()

if MODEL_KIND not in {"memory", "chunk_local"}:
    raise ValueError("LLM_LAB_MODEL_KIND must be 'memory' or 'chunk_local'.")
if NUM_FACTS > NUM_KEYS or NUM_FACTS > NUM_VALUES:
    raise ValueError("NUM_FACTS must fit inside both NUM_KEYS and NUM_VALUES.")


class CausalSelfAttention(nn.Module):
    """Apply masked multi-head self-attention inside one chunk."""

    def __init__(self):
        """Create the query, key, value, and output projections."""
        super().__init__()
        self.num_heads = NUM_HEADS
        self.head_dim = EMBEDDING_DIM // NUM_HEADS
        self.q_proj = nn.Linear(EMBEDDING_DIM, EMBEDDING_DIM, bias=False)
        self.k_proj = nn.Linear(EMBEDDING_DIM, EMBEDDING_DIM, bias=False)
        self.v_proj = nn.Linear(EMBEDDING_DIM, EMBEDDING_DIM, bias=False)
        self.out_proj = nn.Linear(EMBEDDING_DIM, EMBEDDING_DIM, bias=False)

    def split_heads(self, x: torch.Tensor) -> torch.Tensor:
        """Reshape embeddings into separate attention heads."""
        batch_size, chunk_size, _ = x.shape
        return x.reshape(batch_size, chunk_size, self.num_heads, self.head_dim).swapaxes(1, 2)

    def merge_heads(self, x: torch.Tensor) -> torch.Tensor:
        """Merge attention heads back into one embedding axis."""
        batch_size, _, chunk_size, _ = x.shape
        return x.swapaxes(1, 2).reshape(batch_size, chunk_size, self.num_heads * self.head_dim)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Return attention outputs for one batch of chunk embeddings."""
        chunk_size = x.shape[1]

        queries = self.split_heads(self.q_proj(x))
        keys = self.split_heads(self.k_proj(x))
        values = self.split_heads(self.v_proj(x))

        attention_scores = queries @ keys.transpose(-2, -1)
        attention_scores = attention_scores / math.sqrt(self.head_dim)

        causal_mask = torch.triu(
            torch.ones(chunk_size, chunk_size, dtype=torch.bool, device=x.device),
            diagonal=1,
        )
        attention_scores = attention_scores.masked_fill(causal_mask, -torch.inf)

        attention_weights = F.softmax(attention_scores, dim=-1)
        attended_values = self.merge_heads(attention_weights @ values)
        return self.out_proj(attended_values)


class SparseLatentAddressRead(nn.Module):
    """Read runtime memory values from each token's nearest runtime address slots."""

    def __init__(self):
        """Create the token-to-address query and output projections."""
        super().__init__()
        self.q_proj = nn.Linear(EMBEDDING_DIM, ADDRESS_DIM, bias=False)
        self.out_proj = nn.Linear(EMBEDDING_DIM, EMBEDDING_DIM, bias=False)

    def forward(
        self,
        x: torch.Tensor,
        memory_addresses: torch.Tensor,
        memory_values: torch.Tensor,
    ) -> torch.Tensor:
        """Return sparse top-k memory read residuals for one chunk."""
        queries = F.normalize(self.q_proj(x), dim=-1)
        addresses = F.normalize(memory_addresses, dim=-1)

        read_scores = queries @ addresses.transpose(1, 2)
        read_scores = read_scores / READ_TEMPERATURE
        top_scores, top_indices = torch.topk(
            read_scores,
            k=TOP_K_MEMORY_READS,
            dim=-1,
        )
        read_weights = F.softmax(top_scores, dim=-1)

        batch_size, _, _ = x.shape
        batch_indices = torch.arange(batch_size, device=x.device).view(batch_size, 1, 1)
        selected_memory_values = memory_values[batch_indices, top_indices]
        read_values = (read_weights[..., None] * selected_memory_values).sum(dim=2)
        return self.out_proj(read_values)


class RuntimeMemoryWrite(nn.Module):
    """Write memory values, move addresses, and track bounded slot usage."""

    def __init__(self):
        """Create projections for write addresses, values, address deltas, and gates."""
        super().__init__()
        self.q_proj = nn.Linear(EMBEDDING_DIM, ADDRESS_DIM, bias=False)
        self.value_proj = nn.Linear(EMBEDDING_DIM, EMBEDDING_DIM, bias=False)
        self.address_delta_proj = nn.Linear(EMBEDDING_DIM, ADDRESS_DIM, bias=False)
        self.gate_proj = nn.Linear(EMBEDDING_DIM, 1)
        self.allocation_gate_proj = nn.Linear(EMBEDDING_DIM, 1)

    def forward(
        self,
        x: torch.Tensor,
        memory_addresses: torch.Tensor,
        memory_values: torch.Tensor,
        memory_usage: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """Return updated memory state and write diagnostics."""
        if WRITE_GRANULARITY == "chunk":
            return self._forward_chunk(x, memory_addresses, memory_values, memory_usage)
        if WRITE_GRANULARITY == "token":
            return self._forward_token(x, memory_addresses, memory_values, memory_usage)
        raise ValueError("LLM_LAB_WRITE_GRANULARITY must be 'chunk' or 'token'.")

    def _forward_chunk(
        self,
        x: torch.Tensor,
        memory_addresses: torch.Tensor,
        memory_values: torch.Tensor,
        memory_usage: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """Update memory once per chunk, matching the previous write path as the control."""
        write_queries = F.normalize(self.q_proj(x), dim=-1)
        addresses = F.normalize(memory_addresses, dim=-1)
        write_scores = write_queries @ addresses.transpose(1, 2)
        allocation_logits = self.allocation_gate_proj(x)
        write_weights, allocation_gates = self._write_distribution(
            write_scores,
            memory_usage,
            allocation_logits,
        )
        write_gates = torch.sigmoid(self.gate_proj(x))
        write_weights = write_weights * write_gates

        value_updates = self.value_proj(x)
        slot_update_weight = write_weights.sum(dim=1)
        slot_update_sum = write_weights.transpose(1, 2) @ value_updates
        slot_updates = slot_update_sum / slot_update_weight.clamp_min(1e-6)[..., None]
        slot_gates = 1.0 - torch.exp(-slot_update_weight)
        memory_values = torch.lerp(memory_values, slot_updates, slot_gates[..., None])
        memory_usage = update_memory_usage(memory_usage, slot_gates)

        address_delta_values = torch.tanh(self.address_delta_proj(x))
        slot_delta_sum = write_weights.transpose(1, 2) @ address_delta_values
        slot_deltas = slot_delta_sum / slot_update_weight.clamp_min(1e-6)[..., None]
        memory_addresses = move_addresses(memory_addresses, addresses, slot_gates, slot_deltas)
        address_movement = (memory_addresses - addresses).norm(dim=-1).mean()

        return (
            memory_values,
            memory_addresses,
            memory_usage,
            address_movement,
            allocation_gates.mean(),
            write_gates.mean(),
        )

    def _forward_token(
        self,
        x: torch.Tensor,
        memory_addresses: torch.Tensor,
        memory_values: torch.Tensor,
        memory_usage: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """Update memory after each token so allocation can see newly occupied slots."""
        address_movements = []
        allocation_gates = []
        write_gates = []
        value_updates = self.value_proj(x)
        address_delta_values = torch.tanh(self.address_delta_proj(x))
        projected_queries = self.q_proj(x)
        allocation_logits = self.allocation_gate_proj(x)

        for token_index in range(x.size(1)):
            addresses = F.normalize(memory_addresses, dim=-1)
            write_query = F.normalize(projected_queries[:, token_index], dim=-1)
            write_scores = (write_query[:, None, :] @ addresses.transpose(1, 2)).squeeze(1)
            write_weights, allocation_gate = self._write_distribution(
                write_scores[:, None, :],
                memory_usage,
                allocation_logits[:, token_index : token_index + 1],
            )
            write_weights = write_weights.squeeze(1)
            allocation_gate = allocation_gate.squeeze(1)
            write_gate = torch.sigmoid(self.gate_proj(x[:, token_index])).squeeze(-1)
            write_weights = write_weights * write_gate[:, None]

            slot_gates = 1.0 - torch.exp(-write_weights)
            token_values = value_updates[:, token_index]
            memory_values = torch.lerp(memory_values, token_values[:, None, :], slot_gates[..., None])
            memory_usage = update_memory_usage(memory_usage, slot_gates)

            token_deltas = address_delta_values[:, token_index]
            memory_addresses = move_addresses(
                memory_addresses,
                addresses,
                slot_gates,
                token_deltas[:, None, :].expand_as(memory_addresses),
            )
            address_movements.append((memory_addresses - addresses).norm(dim=-1).mean())
            allocation_gates.append(allocation_gate.mean())
            write_gates.append(write_gate.mean())

        return (
            memory_values,
            memory_addresses,
            memory_usage,
            torch.stack(address_movements).mean(),
            torch.stack(allocation_gates).mean(),
            torch.stack(write_gates).mean(),
        )

    def _write_distribution(
        self,
        write_scores: torch.Tensor,
        memory_usage: torch.Tensor,
        allocation_logits: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Return write weights after applying the selected allocation policy."""
        write_scores = write_scores / WRITE_TEMPERATURE
        allocation_gates = torch.sigmoid(allocation_logits)
        if ALLOCATION_MODE == "off":
            write_weights = F.softmax(write_scores, dim=-1)
            return sparsify_write_weights(write_weights), allocation_gates.new_zeros(allocation_gates.shape)

        if ALLOCATION_MODE == "usage_penalty":
            penalized_scores = write_scores - ALLOCATION_STRENGTH * allocation_gates * memory_usage[:, None, :]
            write_weights = F.softmax(penalized_scores, dim=-1)
            return sparsify_write_weights(write_weights), allocation_gates

        if ALLOCATION_MODE == "free_mixture":
            similarity_weights = F.softmax(write_scores, dim=-1)
            free_scores = (1.0 - memory_usage).clamp_min(0.0) / ALLOCATION_TEMPERATURE
            free_weights = F.softmax(free_scores, dim=-1)[:, None, :]
            mix = (ALLOCATION_STRENGTH * allocation_gates).clamp(0.0, 1.0)
            write_weights = (1.0 - mix) * similarity_weights + mix * free_weights
            return sparsify_write_weights(write_weights), allocation_gates

        raise ValueError("LLM_LAB_ALLOCATION_MODE must be 'off', 'usage_penalty', or 'free_mixture'.")


def sparsify_write_weights(write_weights: torch.Tensor) -> torch.Tensor:
    """Keep only the strongest write targets when sparse writes are enabled."""
    if TOP_K_MEMORY_WRITES <= 0 or TOP_K_MEMORY_WRITES >= write_weights.size(-1):
        return write_weights
    top_weights, top_indices = torch.topk(write_weights, k=TOP_K_MEMORY_WRITES, dim=-1)
    sparse_weights = torch.zeros_like(write_weights).scatter(-1, top_indices, top_weights)
    return sparse_weights / sparse_weights.sum(dim=-1, keepdim=True).clamp_min(1e-6)


def update_memory_usage(memory_usage: torch.Tensor, slot_gates: torch.Tensor) -> torch.Tensor:
    """Mark slots as occupied in a bounded way while optionally decaying old usage."""
    memory_usage = memory_usage * USAGE_DECAY
    memory_usage = memory_usage + (1.0 - memory_usage) * slot_gates
    return memory_usage.clamp(0.0, 1.0)


def move_addresses(
    memory_addresses: torch.Tensor,
    normalized_addresses: torch.Tensor,
    slot_gates: torch.Tensor,
    slot_deltas: torch.Tensor,
) -> torch.Tensor:
    """Apply bounded address movement or return normalized addresses when disabled."""
    if ADDRESS_UPDATE_SCALE == 0.0:
        return normalized_addresses
    return F.normalize(
        memory_addresses + ADDRESS_UPDATE_SCALE * slot_gates[..., None] * slot_deltas,
        dim=-1,
    )


def make_initial_addresses() -> torch.Tensor:
    """Create the base address table for the selected geometry control."""
    if ADDRESS_INIT_MODE == "learned_random":
        return torch.randn(NUM_MEMORY_SLOTS, ADDRESS_DIM)

    if ADDRESS_INIT_MODE in {"learned_unit_random", "fixed_random"}:
        return F.normalize(torch.randn(NUM_MEMORY_SLOTS, ADDRESS_DIM), dim=-1)

    if ADDRESS_INIT_MODE == "fixed_spread":
        frames = []
        eye = torch.eye(ADDRESS_DIM)
        frames.extend([eye, -eye])
        generator = torch.Generator().manual_seed(0)
        while sum(frame.size(0) for frame in frames) < NUM_MEMORY_SLOTS:
            random_matrix = torch.randn(ADDRESS_DIM, ADDRESS_DIM, generator=generator)
            q, _ = torch.linalg.qr(random_matrix)
            frames.extend([q.T, -q.T])
        addresses = torch.cat(frames, dim=0)[:NUM_MEMORY_SLOTS]
        return F.normalize(addresses, dim=-1)

    raise ValueError(
        "LLM_LAB_ADDRESS_INIT_MODE must be one of "
        "'learned_random', 'learned_unit_random', 'fixed_random', or 'fixed_spread'."
    )


def address_geometry_stats(addresses: torch.Tensor) -> dict[str, float]:
    """Summarize how close each address is to its nearest neighbor."""
    normalized_addresses = F.normalize(addresses.detach().cpu(), dim=-1)
    cosine = normalized_addresses @ normalized_addresses.T
    diagonal_mask = torch.eye(cosine.size(0), dtype=torch.bool)
    off_diagonal = cosine.masked_fill(diagonal_mask, -torch.inf)
    nearest = off_diagonal.max(dim=-1).values
    return {
        "mean_nearest_cosine": float(nearest.mean().item()),
        "max_nearest_cosine": float(nearest.max().item()),
        "min_nearest_cosine": float(nearest.min().item()),
    }


def normalized_usage_entropy(memory_usage: torch.Tensor) -> torch.Tensor:
    """Return average slot-usage entropy normalized to the range 0 to 1."""
    usage_sum = memory_usage.sum(dim=-1, keepdim=True)
    usage_probs = memory_usage / usage_sum.clamp_min(1e-6)
    entropy = -(usage_probs * usage_probs.clamp_min(1e-8).log()).sum(dim=-1)
    has_usage = usage_sum.squeeze(-1) > 1e-6
    entropy = torch.where(has_usage, entropy / math.log(NUM_MEMORY_SLOTS), entropy.new_zeros(()))
    return entropy.mean()


class FeedForward(nn.Module):
    """Project up, apply a nonlinearity, and project back down."""

    def __init__(self):
        """Create the two linear layers of the MLP."""
        super().__init__()
        self.hidden = nn.Linear(EMBEDDING_DIM, HIDDEN_DIM)
        self.out = nn.Linear(HIDDEN_DIM, EMBEDDING_DIM)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Return the feed-forward block output."""
        x = F.gelu(self.hidden(x))
        return self.out(x)


class RMSNorm(nn.Module):
    """Scale activations by their root-mean-square magnitude."""

    def __init__(self):
        """Create the learned scale parameter."""
        super().__init__()
        self.weight = nn.Parameter(torch.ones(EMBEDDING_DIM))
        self.eps = 1e-5

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Normalize one embedding vector and apply the learned scale."""
        scale = torch.rsqrt(x.pow(2).mean(dim=-1, keepdim=True) + self.eps)
        return x * scale * self.weight


class DecoderBlock(nn.Module):
    """Apply local attention, memory reading, and one MLP block."""

    def __init__(self):
        """Create the norms, attention, memory, and feed-forward sublayers."""
        super().__init__()
        self.attention_norm = RMSNorm()
        self.attention = CausalSelfAttention()
        self.memory_read_norm = RMSNorm()
        self.memory_read = SparseLatentAddressRead()
        self.feed_forward_norm = RMSNorm()
        self.feed_forward = FeedForward()

    def forward(
        self,
        x: torch.Tensor,
        memory_addresses: torch.Tensor,
        memory_values: torch.Tensor,
    ) -> torch.Tensor:
        """Return the residual output of one decoder block."""
        x = x + self.attention(self.attention_norm(x))
        x = x + self.memory_read(self.memory_read_norm(x), memory_addresses, memory_values)
        x = x + self.feed_forward(self.feed_forward_norm(x))
        return x


class ChunkLocalDecoderBlock(nn.Module):
    """Apply local attention and one MLP block without memory."""

    def __init__(self):
        """Create the norms, attention, and feed-forward sublayers."""
        super().__init__()
        self.attention_norm = RMSNorm()
        self.attention = CausalSelfAttention()
        self.feed_forward_norm = RMSNorm()
        self.feed_forward = FeedForward()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Return the residual output of one no-memory decoder block."""
        x = x + self.attention(self.attention_norm(x))
        x = x + self.feed_forward(self.feed_forward_norm(x))
        return x


class Decoder(nn.Module):
    """Stack decoder blocks and update runtime memory after each chunk."""

    def __init__(self) -> None:
        """Create the block stack, base addresses, and write module."""
        super().__init__()
        block_cls = DecoderBlock if MODEL_KIND == "memory" else ChunkLocalDecoderBlock
        self.blocks = nn.ModuleList([block_cls() for _ in range(NUM_BLOCKS)])
        self.out_norm = RMSNorm()
        if MODEL_KIND == "memory":
            initial_addresses = make_initial_addresses()
            if ADDRESS_INIT_MODE in {"learned_random", "learned_unit_random"}:
                self.memory_addresses = nn.Parameter(initial_addresses)
            else:
                self.register_buffer("memory_addresses", initial_addresses)
            self.memory_write_norm = RMSNorm()
            self.memory_write = RuntimeMemoryWrite()

    def forward(
        self,
        x: torch.Tensor,
        memory_addresses: torch.Tensor,
        memory_values: torch.Tensor,
        memory_usage: torch.Tensor,
    ) -> tuple[
        torch.Tensor,
        torch.Tensor,
        torch.Tensor,
        torch.Tensor,
        torch.Tensor,
        torch.Tensor,
        torch.Tensor,
        torch.Tensor,
    ]:
        """Run one chunk and return updated runtime memory state."""
        if MODEL_KIND == "chunk_local":
            for block in self.blocks:
                x = block(x)
            x = self.out_norm(x)
            zero = x.new_tensor(0.0)
            return (
                x,
                memory_values,
                memory_addresses,
                memory_usage,
                zero,
                zero,
                zero,
                zero,
                zero,
            )

        for block in self.blocks:
            x = block(x, memory_addresses, memory_values)

        x = self.out_norm(x)
        (
            memory_values,
            memory_addresses,
            memory_usage,
            address_movement,
            allocation_gate,
            write_gate,
        ) = self.memory_write(
            self.memory_write_norm(x),
            memory_addresses,
            memory_values,
            memory_usage,
        )
        usage_entropy = normalized_usage_entropy(memory_usage)
        return (
            x,
            memory_values,
            memory_addresses,
            memory_usage,
            address_movement,
            memory_usage.mean(),
            usage_entropy,
            allocation_gate,
            write_gate,
        )


class LanguageModel(nn.Module):
    """Embed tokens, process chunks with controlled address geometry, and predict logits."""

    def __init__(self, vocab_size: int):
        """Create the embeddings and decoder."""
        super().__init__()
        self.vocab_size = vocab_size
        self.token_embedding = nn.Embedding(vocab_size, EMBEDDING_DIM)
        self.position_embedding = nn.Embedding(SEQUENCE_LEN, EMBEDDING_DIM)
        self.decoder = Decoder()
        self.last_address_movement = torch.tensor(0.0)
        self.last_mean_memory_usage = torch.tensor(0.0)
        self.last_usage_entropy = torch.tensor(0.0)
        self.last_mean_allocation_gate = torch.tensor(0.0)
        self.last_mean_write_gate = torch.tensor(0.0)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Return next-token logits for one batch of token ids."""
        batch_size, sequence_len = x.shape
        num_chunks = sequence_len // CHUNK_SIZE
        positions = torch.arange(sequence_len, device=x.device)
        position_embeddings = self.position_embedding(positions).reshape(
            1, num_chunks, CHUNK_SIZE, EMBEDDING_DIM
        )

        x = x.reshape(batch_size, num_chunks, CHUNK_SIZE)
        x = self.token_embedding(x) + position_embeddings

        memory_values = x.new_zeros(batch_size, NUM_MEMORY_SLOTS, EMBEDDING_DIM)
        memory_usage = x.new_zeros(batch_size, NUM_MEMORY_SLOTS)
        if MODEL_KIND == "memory":
            memory_addresses = F.normalize(
                self.decoder.memory_addresses.unsqueeze(0).expand(batch_size, -1, -1).clone(),
                dim=-1,
            )
        else:
            memory_addresses = x.new_zeros(batch_size, NUM_MEMORY_SLOTS, ADDRESS_DIM)
        address_movements = []
        memory_usages = []
        usage_entropies = []
        allocation_gates = []
        write_gates = []
        chunk_outputs: list[torch.Tensor] = []
        for chunk_index in range(num_chunks):
            (
                chunk_output,
                memory_values,
                memory_addresses,
                memory_usage,
                address_movement,
                mean_memory_usage,
                usage_entropy,
                allocation_gate,
                write_gate,
            ) = self.decoder(
                x[:, chunk_index],
                memory_addresses,
                memory_values,
                memory_usage,
            )
            address_movements.append(address_movement)
            memory_usages.append(mean_memory_usage)
            usage_entropies.append(usage_entropy)
            allocation_gates.append(allocation_gate)
            write_gates.append(write_gate)
            chunk_outputs.append(chunk_output)

        self.last_address_movement = torch.stack(address_movements).mean().detach()
        self.last_mean_memory_usage = torch.stack(memory_usages).mean().detach()
        self.last_usage_entropy = torch.stack(usage_entropies).mean().detach()
        self.last_mean_allocation_gate = torch.stack(allocation_gates).mean().detach()
        self.last_mean_write_gate = torch.stack(write_gates).mean().detach()
        x = torch.stack(chunk_outputs, dim=1)
        x = x @ self.token_embedding.weight.T
        x = x.reshape(batch_size, sequence_len, self.vocab_size)
        return x


def build_vocab() -> tuple[list[str], dict[str, int]]:
    """Build the small synthetic vocabulary for delayed recall."""
    vocab_tokens = [
        PAD_TOKEN,
        BOS_TOKEN,
        EOS_TOKEN,
        STORE_TOKEN,
        QUERY_TOKEN,
        ANSWER_TOKEN,
    ]
    vocab_tokens.extend([f"K{idx}" for idx in range(NUM_KEYS)])
    vocab_tokens.extend([f"V{idx}" for idx in range(NUM_VALUES)])
    vocab_tokens.extend([f"N{idx}" for idx in range(NUM_NOISE_TOKENS)])
    token_to_id = {token: idx for idx, token in enumerate(vocab_tokens)}
    return vocab_tokens, token_to_id


def build_example(token_to_id: dict[str, int]) -> tuple[list[int], list[int], list[int]]:
    """Create one delayed key-value recall sequence and answer positions."""
    fact_tokens = [token_to_id[BOS_TOKEN]]
    key_tokens = [token_to_id[f"K{idx}"] for idx in range(NUM_KEYS)]
    value_tokens = [token_to_id[f"V{idx}"] for idx in range(NUM_VALUES)]
    noise_tokens = [token_to_id[f"N{idx}"] for idx in range(NUM_NOISE_TOKENS)]

    chosen_keys = random.sample(key_tokens, NUM_FACTS)
    chosen_values = random.sample(value_tokens, NUM_FACTS)
    key_value_pairs = list(zip(chosen_keys, chosen_values))

    for key_id, value_id in key_value_pairs:
        fact_tokens.extend([token_to_id[STORE_TOKEN], key_id, value_id])

    query_pairs = key_value_pairs[:]
    random.shuffle(query_pairs)
    suffix_len = len(query_pairs) * 4 + 1

    full_sequence_len = SEQUENCE_LEN + 1
    filler_len = full_sequence_len - len(fact_tokens) - suffix_len
    if filler_len < 0:
        raise ValueError("SEQUENCE_LEN is too small for the delayed-recall layout.")

    filler_tokens = random.choices(noise_tokens, k=filler_len)
    token_ids = fact_tokens + filler_tokens
    answer_positions = []
    for query_key_id, query_value_id in query_pairs:
        token_ids.extend([token_to_id[QUERY_TOKEN], query_key_id, token_to_id[ANSWER_TOKEN]])
        answer_positions.append(len(token_ids) - 1)
        token_ids.append(query_value_id)
    token_ids.append(token_to_id[EOS_TOKEN])
    return token_ids, answer_positions, chosen_values


def sample_batch(
    token_to_id: dict[str, int],
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """Sample delayed-recall input, target, answer-mask, and candidate tensors."""
    inputs = torch.empty((BATCH_SIZE, SEQUENCE_LEN), dtype=torch.long, device=DEVICE)
    targets = torch.empty((BATCH_SIZE, SEQUENCE_LEN), dtype=torch.long, device=DEVICE)
    answer_mask = torch.zeros((BATCH_SIZE, SEQUENCE_LEN), dtype=torch.float32, device=DEVICE)
    candidate_values = torch.empty((BATCH_SIZE, NUM_FACTS), dtype=torch.long, device=DEVICE)

    for batch_index in range(BATCH_SIZE):
        token_ids, answer_positions, example_candidate_values = build_example(token_to_id)
        inputs[batch_index] = torch.tensor(token_ids[:-1], dtype=torch.long, device=DEVICE)
        targets[batch_index] = torch.tensor(token_ids[1:], dtype=torch.long, device=DEVICE)
        answer_mask[batch_index, answer_positions] = 1.0
        candidate_values[batch_index] = torch.tensor(
            example_candidate_values,
            dtype=torch.long,
            device=DEVICE,
        )

    return inputs, targets, answer_mask, candidate_values


def answer_metrics(
    logits: torch.Tensor,
    targets: torch.Tensor,
    answer_mask: torch.Tensor,
    candidate_values: torch.Tensor,
) -> tuple[float, float]:
    """Return exact answer accuracy and candidate-value accuracy."""
    predictions = logits.argmax(dim=-1)
    is_answer = answer_mask.bool()
    predicted_answers = predictions[is_answer]
    target_answers = targets[is_answer]
    exact_accuracy = (predicted_answers == target_answers).float().mean()
    candidate_hits = (predictions[:, :, None] == candidate_values[:, None, :]).any(dim=-1)
    candidate_accuracy = candidate_hits[is_answer].float().mean()
    return float(exact_accuracy.item()), float(candidate_accuracy.item())


@torch.no_grad()
def estimate_metrics(
    model: LanguageModel,
    token_to_id: dict[str, int],
) -> tuple[float, float, float, float, float, float, float, float]:
    """Estimate answer loss, answer accuracy, and memory-write diagnostics."""
    losses: list[float] = []
    exact_accuracies: list[float] = []
    candidate_accuracies: list[float] = []
    address_movements: list[float] = []
    memory_usages: list[float] = []
    usage_entropies: list[float] = []
    allocation_gates: list[float] = []
    write_gates: list[float] = []
    model.eval()

    for _ in range(EVAL_BATCHES):
        inputs, targets, answer_mask, candidate_values = sample_batch(token_to_id)
        logits = model(inputs)
        loss = loss_fn(logits, targets, answer_mask)
        exact_accuracy, candidate_accuracy = answer_metrics(
            logits,
            targets,
            answer_mask,
            candidate_values,
        )
        losses.append(float(loss.item()))
        exact_accuracies.append(exact_accuracy)
        candidate_accuracies.append(candidate_accuracy)
        address_movements.append(float(model.last_address_movement.item()))
        memory_usages.append(float(model.last_mean_memory_usage.item()))
        usage_entropies.append(float(model.last_usage_entropy.item()))
        allocation_gates.append(float(model.last_mean_allocation_gate.item()))
        write_gates.append(float(model.last_mean_write_gate.item()))

    model.train()
    return (
        sum(losses) / len(losses),
        sum(exact_accuracies) / len(exact_accuracies),
        sum(candidate_accuracies) / len(candidate_accuracies),
        sum(address_movements) / len(address_movements),
        sum(memory_usages) / len(memory_usages),
        sum(usage_entropies) / len(usage_entropies),
        sum(allocation_gates) / len(allocation_gates),
        sum(write_gates) / len(write_gates),
    )


def loss_fn(logits: torch.Tensor, targets: torch.Tensor, answer_mask: torch.Tensor) -> torch.Tensor:
    """Compute next-token cross-entropy only at the answer position."""
    per_token_loss = F.cross_entropy(
        logits.reshape(-1, logits.size(-1)),
        targets.reshape(-1),
        reduction="none",
    ).reshape_as(targets)
    return (per_token_loss * answer_mask).sum() / answer_mask.sum()


def main() -> None:
    """Train one longer-context delayed-recall pressure-test variant."""
    random.seed(SEED)
    torch.manual_seed(SEED)

    vocab_tokens, token_to_id = build_vocab()
    model = LanguageModel(len(vocab_tokens)).to(DEVICE)
    geometry_stats = (
        address_geometry_stats(model.decoder.memory_addresses)
        if MODEL_KIND == "memory"
        else None
    )
    optimizer = torch.optim.AdamW(model.parameters(), lr=LEARNING_RATE)
    metric_tracker = MemoryMetricTracker(
        candidate_guess_exact_baseline=CANDIDATE_GUESS_BASELINE,
        random_value_exact_baseline=RANDOM_VALUE_EXACT_BASELINE,
        random_value_candidate_baseline=RANDOM_VALUE_CANDIDATE_BASELINE,
    )
    print(f"experiment_name={EXPERIMENT_NAME}")
    print(f"device={DEVICE}")
    print(f"model_kind={MODEL_KIND}")
    print(f"seed={SEED}")
    print(f"sequence_len={SEQUENCE_LEN}")
    print(f"chunk_size={CHUNK_SIZE}")
    print(f"num_chunks={SEQUENCE_LEN // CHUNK_SIZE}")
    print(f"num_facts={NUM_FACTS}")
    print(f"num_keys={NUM_KEYS}")
    print(f"num_values={NUM_VALUES}")
    print(f"num_noise_tokens={NUM_NOISE_TOKENS}")
    print(f"train_steps={TRAIN_STEPS}")
    print(f"eval_interval={EVAL_INTERVAL}")
    print(f"eval_batches={EVAL_BATCHES}")
    print(f"address_init_mode={ADDRESS_INIT_MODE}")
    print(f"num_memory_slots={NUM_MEMORY_SLOTS}")
    print(f"top_k_memory_reads={TOP_K_MEMORY_READS}")
    print(f"top_k_memory_writes={TOP_K_MEMORY_WRITES}")
    print(f"address_update_scale={ADDRESS_UPDATE_SCALE}")
    print(f"allocation_mode={ALLOCATION_MODE}")
    print(f"allocation_strength={ALLOCATION_STRENGTH}")
    print(f"allocation_temperature={ALLOCATION_TEMPERATURE}")
    print(f"usage_decay={USAGE_DECAY}")
    print(f"write_granularity={WRITE_GRANULARITY}")
    if geometry_stats is not None:
        print(
            "address_geometry="
            f"mean_nearest_cosine={geometry_stats['mean_nearest_cosine']:.6f} "
            f"max_nearest_cosine={geometry_stats['max_nearest_cosine']:.6f} "
            f"min_nearest_cosine={geometry_stats['min_nearest_cosine']:.6f}"
        )
    print(
        f"candidate_guess_exact_baseline={CANDIDATE_GUESS_BASELINE:.4f} "
        f"random_value_exact_baseline={RANDOM_VALUE_EXACT_BASELINE:.4f} "
        f"random_value_candidate_baseline={RANDOM_VALUE_CANDIDATE_BASELINE:.4f}"
    )

    for step in range(TRAIN_STEPS):
        inputs, targets, answer_mask, _ = sample_batch(token_to_id)
        logits = model(inputs)
        loss = loss_fn(logits, targets, answer_mask)

        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

        should_log = step == 0 or (step + 1) % EVAL_INTERVAL == 0 or step == TRAIN_STEPS - 1
        if should_log:
            (
                answer_loss,
                exact_accuracy,
                candidate_accuracy,
                address_movement,
                memory_usage,
                usage_entropy,
                allocation_gate,
                write_gate,
            ) = estimate_metrics(model, token_to_id)
            metric_tracker.log(
                step=step + 1,
                batch_answer_loss=loss.item(),
                eval_answer_loss=answer_loss,
                eval_exact_answer_accuracy=exact_accuracy,
                eval_candidate_value_accuracy=candidate_accuracy,
                eval_mean_address_movement=address_movement,
                eval_mean_memory_usage=memory_usage,
                eval_usage_entropy=usage_entropy,
                eval_mean_allocation_gate=allocation_gate,
                eval_mean_write_gate=write_gate,
            )

    artifacts = metric_tracker.save(script_path=Path(f"{EXPERIMENT_NAME}.py"))
    for artifact_name, artifact_path in artifacts.items():
        print(f"{artifact_name}={artifact_path}")


if __name__ == "__main__":
    main()
