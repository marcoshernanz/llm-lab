"""Memory architecture experiment 016: address-geometry stabilization."""
...
ADDRESS_UPDATE_SCALE = float(os.environ.get("LLM_LAB_ADDRESS_UPDATE_SCALE", "0.0"))
...
class RuntimeMemoryWrite(nn.Module):
    """Write memory values and optionally move runtime addresses by a bounded amount."""
    ...
    def forward(
        ...
        if ADDRESS_UPDATE_SCALE == 0.0:
            address_movement = x.new_tensor(0.0)
            return memory_values, addresses, address_movement
...
def main() -> None:
    """Train the address-geometry control on multi-query binding recall."""
    random.seed(SEED)
