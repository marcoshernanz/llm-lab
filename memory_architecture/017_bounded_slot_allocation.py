"""Memory architecture experiment 017: bounded slot allocation."""
...
BATCH_SIZE = 64
LEARNING_RATE = 3e-3
TRAIN_STEPS = int(os.environ.get("LLM_LAB_TRAIN_STEPS", "4000"))
EVAL_INTERVAL = int(os.environ.get("LLM_LAB_EVAL_INTERVAL", "200"))
...
def main() -> None:
    """Train the bounded slot-allocation model on multi-query binding recall."""
    random.seed(SEED)
    torch.manual_seed(SEED)

    vocab_tokens, token_to_id = build_vocab()
    model = LanguageModel(len(vocab_tokens)).to(DEVICE)
    geometry_stats = address_geometry_stats(model.decoder.memory_addresses)
