"""Train the milestone-030 explicit-sharding multi-core baseline with run metadata."""
...
def parse_args() -> ExperimentConfig:
    ...
    parser = argparse.ArgumentParser(
        description="Train one milestone-030 explicit-sharding TPU multicore point."
    )
    parser.add_argument(
        "--token-shard-root",
        type=Path,
...
def make_replicated_sharding(mesh: jax.sharding.Mesh) -> NamedSharding:
    """Describe a value that should be replicated across the whole mesh."""
...
def place_batch_on_mesh(
    ...
    *,
    ...
    if require_even_sharding:
        raise ValueError(
            "Batch size must be divisible by the number of devices for training. "
...
def train_step(
    ...
    """Run one Optax AdamW step on a sharded batch of token examples."""
...
def train_chunk(
    ...
    *,
    ...
    """Average several random sharded training batches into one logged chunk."""
...
def evaluate_positions_on_mesh(
    ...
    *,
    ...
    """Average loss over a chosen set of positions using sharded eval batches when possible."""
    ...
    for batch_start in range(0, start_positions.shape[0], batch_size):
        ...
        batch_loss = evaluate_batch_loss(model, placed_input_ids, placed_target_ids)
...
def main() -> None:
    """Run one milestone-030 explicit-sharding TPU multicore point end to end."""
    config = parse_args()
    ...
    replicated_sharding = make_replicated_sharding(mesh)
