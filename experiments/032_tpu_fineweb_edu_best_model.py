...
class ExperimentConfig:
    ...
    def validate(self) -> None:
        ...
        if (
            self.eval_interval_steps != 0
            and self.eval_interval_steps % self.train_chunk_length != 0
        ):
...
def main() -> None:
    ...
    with jax.set_mesh(mesh):
        ...
        for chunk_index, _ in enumerate(range(0, config.train_steps, config.train_chunk_length)):
            ...
            should_evaluate = (
                config.eval_interval_steps > 0 and current_step % config.eval_interval_steps == 0
            )
            if should_evaluate:
